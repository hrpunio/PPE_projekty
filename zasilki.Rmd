...
Plik `socjalne2020.csv` zawiera miesięczne dane nt wysokoci zasiku w Polsce w okresie
...
Analizując wykres dostrzegamy brak wahania sezonowego oraz trend. 
Dostrzegamy gwałtowny wzrost w okresie kwiecień-wrzesień 2020, byc moze jest to spowodowane druga fala COVID-19
...
wysokosc wyplacanego swiadczenia uzalezniona jest od ustawy, sezonowosc nie ma na to wplywu
...
Ponieważ szereg wykazuje się trendem  porównamy wyniki
...
wybiermy model BROWNA jako najlepiej prognozujący wysokosc wyplacanego zasilku
Reguła predykcji w postaci rekurencyjnej:

dla pierwszego momentu czasowego:

{\displaystyle Y_{1}^{*}=Y_{0},}{\displaystyle Y_{1}^{*}=Y_{0},}
dla następnych:

{\displaystyle Y_{t}^{*}=\alpha Y_{t-1}+(1-\alpha )Y_{t-1}^{*},}{\displaystyle Y_{t}^{*}=\alpha Y_{t-1}+(1-\alpha )Y_{t-1}^{*},}
gdzie:

{\displaystyle Y_{t}^{*}}{\displaystyle Y_{t}^{*}} – wartości prognozy dla danej wartości parametru wygładzania {\displaystyle \alpha ,}\alpha ,
{\displaystyle Y_{t}}Y_t – wartości szeregu,
{\displaystyle \alpha }\alpha  – parametr wyrównywania wykładniczego (współczynnik wygładzania) z przedziału (0,1].
...
```
  ...
  wszystkie prognozy są systematycznie. Być może 
